                               ...
                               equal)
{
  ...
  if (!match.T){
    ## stratification
    means0 <- means1 <- sd0 <- sd1 <- stdf <- matrix(NA,
                                                     ncol=dim(data)[2],
    ...
    if (nlevels(as.factor(cov))   == 1 |
      ...
      if (nlevels(as.factor(cov)) == 2){
        ...
        if (!match.T){ ## if stratification
          ...
          if (max(cov, na.rm=TRUE) > 1){ ## na.rm=TRUE is necessary if cov contains NAs.
            help <- cov
          ...
          if (dim(table.cov)[2] != nlevels(as.factor(treat)) |
            ...
            if (equal){
              ...
              n1 <- length(cov[treat == min(treat, na.rm=TRUE)])
              n0 <- length(cov[treat == max(treat, na.rm=TRUE)])
          ...
          for (j in 1:nlevels(as.factor(index))){
            ...
            if (any(rowSums(table.cov.s[,,j]) == 0) |
              ...
              means0[j+1,i] <- mean(cov[treat == min(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
              means1[j+1,i] <- mean(cov[treat == max(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
              ...
              if (equal){
                ...
                n1 <- length(cov[treat == min(treat, na.rm=TRUE) &  index == j])
                n0 <- length(cov[treat == max(treat, na.rm=TRUE) &  index == j])
          ...
          for (j in 1:nlevels(as.factor(index))){
            ...
            if (any(rowSums(table.cov.s[,,j]) == 0) |
              ...
              means0[j,i] <- mean(cov[treat == min(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
              means1[j,i] <- mean(cov[treat == max(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
        ...
        if (!match.T){ ## stratification
          ...
          for (j in 1:nlevels(as.factor(index))){
            ...
            if (length(cov[treat == min(treat, na.rm=TRUE) & index == j]) == 0 |
              ...
              sd0[j+1,i] <- sd(cov[treat == min(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
              sd1[j+1,i] <- sd(cov[treat == max(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
          ...
          for (j in 1:nlevels(as.factor(index))){
            ...
            if (length(cov[treat == min(treat, na.rm=TRUE) & index == j]) == 0 |
              ...
              sd0[j,i] <- sd(cov[treat == min(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
              sd1[j,i] <- sd(cov[treat == max(treat, na.rm=TRUE) & index == j], na.rm=TRUE)
