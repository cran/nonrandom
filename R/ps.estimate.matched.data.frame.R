ps.estimate.matched.data.frame <- function(object,
                                           resp,
                                           treat         = NULL,
                                           stratum.index = NULL,
                                           match.index   = NULL,
                                           ...
                                           ...)
{
  ...
  if (!is.logical(regr)){
    if (!inherits(regr, "formula")){      
      ...
      if (any(names(as.data.frame(model.frame(regr,d1)))[c(1)] != name.resp))
          stop(paste("Argument 'regr' does not include ", name.resp,
                     " as response.", sep=""))
        
        if (any(names(as.data.frame(model.frame(regr,d1)))[c(2)] != name.treat))
